...
from rich.progress import Progress, TextColumn, TimeElapsedColumn, TimeRemainingColumn, BarColumn, SpinnerColumn
from gas.commons import DO_SAMPLE, GPT_JUDGE, MAX_NEW_TOKENS, MIN_NEW_TOKENS, PENALTY_ALPHA, SEED, TEMPERATURE, TOP_K, TOP_P
...
def build_llm_test_cases(
        ...
        ,limit: int | None) -> EvaluationDataset:
    """
    ...
    with MyProgress(
        ...
        for i in range(total):
            ...
            if Logger().is_verbose and Logger().get_level == "DEBUG":
...
def fetch_results(model: str, task_type: str) -> list[dict[str,any]]:
    ...
    with MyProgress(
        ...
        task = progress.add_task(f"[bold bright_green]Fetching {total} test cases results:[/bold bright_green]", total=total)
...
class evaluationPipeline:
    ...
    def _show_general_information(self):
        ...
        Logger().print_information_table_panel(information
    ...
    def _show_benchmark_summary(self, benchmark_records: list[dict[str,any]]):
        ...
        for metric, values in totals.items():
            summary[metric] = (f"average score: {values["total_score"] / values["count"]}\n"
                               f"success rate: {values["total_success"] / values["count"] * 100}\n"
                               f"total tests: {values['count']}")
            
        Logger().print_information_table_panel(
        [summary]
        ,title=f"[bold bright_green]Benchmark Summary[/bold bright_green]"
        ,border_style="green")
    ...
    def _fetch_metrics(self, prompt_instruction: str) -> list[BaseMetric]:
        ...
        if self.task == BenchmarkType.CHOICE or self.task == BenchmarkType.TF:
            return [
                accuracy_score.ObjectiveAccuracyMetric()
            ]
        else:
            return [
    ...
    def run(self, **kwargs: dict[str,any]):
        ...
        logger.info("Done.")
        Logger().print_panel(f"[yellow][align=left]{str(PROMPT_INSTRUCTION)}[/align][/yellow]",title="[bold bright_yellow]Prompt Instruction[/bold bright_yellow]", border_style="yellow")
        logger.info("Fetching benchmark metrics")
        metrics = self._fetch_metrics(PROMPT_INSTRUCTION)
        log_metrics = "\n".join([m.__name__ for m in metrics])
        Logger().print_panel(log_metrics,title="[bold medium_orchid]Benchmark Metrics[/bold medium_orchid]", border_style="purple",justify="center")
        logger.info("Done.")
